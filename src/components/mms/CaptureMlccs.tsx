...
export function CaptureMlccs() {
  ...
  const handleModify = () => {
    ...
    // Simulate loading existing record
    setModForm({
      ...emptyForm,
      cosSection: "ARTY-01",
      censusNo: modCensus,
      ...
      {mode === "add" ? (
        showAddFull ? (
          <FullEqptForm
            form={addForm}
            setForm={setAddForm}
            lockedFields={["cosSection", "censusNo"]}
            onSave={() => toast.success("Equipment saved successfully")}
            onClear={() => {
        ...
        <FullEqptForm
          ...
          setForm={setModForm}
          lockedFields={["cosSection", "censusNo"]}
          onSave={() => toast.success("Record updated successfully")}
          onClear={() => {
        ...
        <div className="space-y-4 max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-2">
            ...
            <Button variant="secondary" onClick={() => { setModCensus(""); setModNom(""); }}>
              Clear
            </Button>
            <Button variant="destructive" onClick={() => toast("Cancelled")}>
              Cancel
    ...
    <div className="space-y-2 text-xs">
      <FormGrid cols={3}>
        ...
        <FormRow label="Auth/Letter No" required>
          <Input value={form.authLetterNo} onChange={(e) => upd("authLetterNo", e.target.value)} placeholder="Enter Auth/Letter No" />
        </FormRow>
        <FormRow label="Date" required>
          <Input type="date" value={form.date} onChange={(e) => upd("date", e.target.value)} />
        </FormRow>
        <FormRow label="PRF Group" required>
          <SelectField value={form.prfGroup} onChange={(v) => upd("prfGroup", v)} options={["Group A", "Group B", "Group C"]} />
        </FormRow>
        <FormRow label="Item Code" required>
          <SelectField value={form.itemCode} onChange={(v) => upd("itemCode", v)} options={["IC-001", "IC-002", "IC-003"]} />
        </FormRow>
        <FormRow label="Cat/Part No" required>
          <Input value={form.catPartNo} onChange={(e) => upd("catPartNo", e.target.value)} placeholder="Enter Cat/Part No" />
        </FormRow>
        <FormRow label="Accounting Unit" required>
          <SelectField value={form.accountingUnit} onChange={(v) => upd("accountingUnit", v)} options={["NOS", "KG", "LTR", "MTR"]} />
        </FormRow>
        <FormRow label="Item Status" required>
          <SelectField value={form.itemStatus} onChange={(v) => upd("itemStatus", v)} options={["CUR", "OBS", "PHS"]} />
        </FormRow>
        <FormRow label="Item Category" required>
          <SelectField value={form.itemCategory} onChange={(v) => upd("itemCategory", v)} options={["Weapon", "Ammunition", "Vehicle", "Communication"]} />
        </FormRow>
        <FormRow label="Class of Eqpt" required>
          <SelectField value={form.classOfEqpt} onChange={(v) => upd("classOfEqpt", v)} options={["Class I", "Class II", "Class III"]} />
        </FormRow>
        <FormRow label="Country of Origin">
          <Input value={form.countryOfOrigin} onChange={(e) => upd("countryOfOrigin", e.target.value)} placeholder="Search..." />
        </FormRow>
        <FormRow label="Nodal Dte">
          <SelectField value={form.nodalDte} onChange={(v) => upd("nodalDte", v)} options={["DGOS", "DGAS", "DGEME"]} />
        </FormRow>
        <FormRow label="Eqpt Category">
          <SelectField value={form.eqptCategory} onChange={(v) => upd("eqptCategory", v)} options={["A", "B", "C"]} />
        </FormRow>
        <FormRow label="Incl in AIH">
          <SelectField value={form.inclInAih} onChange={(v) => upd("inclInAih", v)} options={["Yes", "No"]} />
        </FormRow>
        <FormRow label="Year of Induction">
          <Input value={form.yearOfInduction} onChange={(e) => upd("yearOfInduction", e.target.value)} />
        </FormRow>
        <FormRow label="Digest Category">
          <SelectField value={form.digestCategory} onChange={(v) => upd("digestCategory", v)} options={["Cat-I", "Cat-II"]} />
        </FormRow>
        <FormRow label="Cost (Rs.)">
          <Input value={form.cost} onChange={(e) => upd("cost", e.target.value)} placeholder="Enter Cost..." />
        </FormRow>
        <FormRow label="Manufacturing Agency">
          <Input value={form.manufacturingAgency} onChange={(e) => upd("manufacturingAgency", e.target.value)} placeholder="Enter Man. Agency..." />
        </FormRow>
        <FormRow label="AHSP Agency">
          <Input value={form.ahspAgency} onChange={(e) => upd("ahspAgency", e.target.value)} placeholder="Enter AHSP Agency..." />
        </FormRow>
        <FormRow label="NATO Stock No (NSN)">
          <Input value={form.natoStockNo} onChange={(e) => upd("natoStockNo", e.target.value)} placeholder="Enter No..." />
        </FormRow>
        <FormRow label="Def Catalogue No (DCAN)">
          <Input value={form.defCatalogueNo} onChange={(e) => upd("defCatalogueNo", e.target.value)} placeholder="Enter No..." />
        </FormRow>
      </FormGrid>
      ...
      <FormGrid cols={2}>
        ...
        <FormRow label="Remarks">
          <Textarea rows={2} value={form.remarks} onChange={(e) => upd("remarks", e.target.value)} placeholder="Enter Your Remarks..." />
        </FormRow>
      </FormGrid>

      <div className="flex flex-wrap justify-center gap-2 pt-2 border-t border-border">
        <Button size="sm" onClick={onSave} className="bg-success hover:bg-success/90 text-success-foreground">
          {primaryLabel}
        </Button>
