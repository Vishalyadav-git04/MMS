...
export function LinkEqptUe() {
  ...
  return (
    <FormPanel title="Linking of Census No with Item Code">
      {!fetched ? (
        <div className="space-y-4 max-w-3xl mx-auto">
          <FormRow label="Census No" required>
            <Input value={censusNo} onChange={(e) => setCensusNo(e.target.value)} placeholder="Search..." />
          </FormRow>
          <FormRow label="Nomenclature" required>
            <Textarea rows={2} value={nomenclature} onChange={(e) => setNomenclature(e.target.value)} placeholder="Search..." />
          </FormRow>
          <div className="flex flex-wrap justify-center gap-2 pt-2">
            <Button onClick={handleFetch} className="bg-primary hover:bg-primary/90">
              Fetch Details
            </Button>
            <Button variant="secondary" onClick={() => { setCensusNo(""); setNomenclature(""); }}>Clear</Button>
            <Button variant="destructive" onClick={() => toast("Cancelled")}>Cancel</Button>
          </div>
        </div>
      ) : (
        <div className="space-y-4 max-w-3xl mx-auto">
          ...
          <FormRow label="Nomenclature" required>
            <Textarea rows={2} value={nomenclature} disabled />
          </FormRow>
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-4 border-t border-border">
            ...
            <Button variant="secondary" onClick={() => setLinkedCode("")}>Clear</Button>
            <Button variant="destructive" onClick={() => setFetched(false)}>Cancel</Button>
          </div>
        </div>
      )}
    </FormPanel>
