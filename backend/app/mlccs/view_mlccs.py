...
Search / list / class-of-eqpt options against MMS_MLCCS_EQPT_MASTER.
...
class MlccsSearchRequest(BaseModel):
    ...
    field: str = Field(
        ...
        description="Nomenclature | Census No | Material No | Cat Part No | COS Sec | PRF Group | Status",
    )
...
def mlccs_options(session: Session = Depends(get_db_session)) -> dict[str, list[dict[str, str]]]:
    ...
    sql = "SELECT DISTINCT class_category FROM MMS_MLCCS_EQPT_MASTER WHERE class_category IS NOT NULL ORDER BY class_category"
...
def _build_where_clause(
    ...
    if body.class_of_eqpt and body.class_of_eqpt.strip():
        sql_where += " AND UPPER(m.class_category) = :ceqpt"
    ...
    if text_val:
        ...
        if field == "Census No":
            sql_where += " AND UPPER(m.census_no) LIKE :qtext"
        elif field == "Material No":
            sql_where += " AND UPPER(m.material_no) LIKE :qtext"
        elif field == "Cat Part No":
            sql_where += " AND UPPER(m.cat_part_no) LIKE :qtext"
        elif field == "COS Sec":
            sql_where += " AND UPPER(m.cos_sec) LIKE :qtext"
        elif field == "PRF Group":
            sql_where += " AND UPPER(m.prf_group) LIKE :qtext"
        elif field == "Status":
            status_codes = []
            if opstatus_map:
                for code, label in opstatus_map.items():
                    if text_val.upper() in label.upper() or text_val.upper() in code.upper():
                        status_codes.append(code.upper())
            if status_codes:
                st_param_names = []
                for st_i, st_code in enumerate(status_codes):
                    st_pname = f"f_st_{st_i}"
                    params[st_pname] = st_code
                    st_param_names.append(f":{st_pname}")
                st_in_list = ", ".join(st_param_names)
                sql_where += f" AND (UPPER(m.op_status) LIKE :qtext OR UPPER(m.item_status) LIKE :qtext OR UPPER(m.op_status) IN ({st_in_list}) OR UPPER(m.item_status) IN ({st_in_list}))"
            else:
                sql_where += " AND (UPPER(m.op_status) LIKE :qtext OR UPPER(m.item_status) LIKE :qtext)"
        else:
            sql_where += " AND UPPER(m.nomen) LIKE :qtext"
        params["qtext"] = q
        ...
        for idx, term in enumerate(terms):
            ...
            if status_codes:
                ...
                for st_i, st_code in enumerate(status_codes):
                    ...
                    st_param_names.append(f":{st_pname}")
                st_in_list = ", ".join(st_param_names)
                status_clause = f" OR UPPER(m.op_status) IN ({st_in_list}) OR UPPER(m.item_status) IN ({st_in_list})"

            sql_where += f""" AND (
                UPPER(m.material_no) LIKE :{param_name} OR
                UPPER(m.census_no) LIKE :{param_name} OR
                UPPER(m.nomen) LIKE :{param_name} OR
                UPPER(m.class_category) LIKE :{param_name} OR
                UPPER(m.cat_part_no) LIKE :{param_name} OR
                UPPER(m.au) LIKE :{param_name} OR
                UPPER(m.cos_sec) LIKE :{param_name} OR
                UPPER(m.prf_group) LIKE :{param_name} OR
                UPPER(m.item_code) LIKE :{param_name} OR
                UPPER(p.item_name) LIKE :{param_name} OR
                UPPER(m.op_status) LIKE :{param_name} OR
                UPPER(m.item_status) LIKE :{param_name}
                {status_clause}
            )"""

    # Relevance ordering: exact match -> prefix match -> default cos_sec, prf_group, item_code, census_no
    if result_q:
        ...
        order_sql = """ORDER BY
            CASE
                WHEN UPPER(m.material_no) = :full_rq_exact OR UPPER(m.census_no) = :full_rq_exact OR UPPER(m.nomen) = :full_rq_exact OR UPPER(m.cat_part_no) = :full_rq_exact OR UPPER(m.class_category) = :full_rq_exact THEN 1
                WHEN UPPER(m.material_no) LIKE :full_rq_pfx OR UPPER(m.census_no) LIKE :full_rq_pfx OR UPPER(m.nomen) LIKE :full_rq_pfx OR UPPER(m.cat_part_no) LIKE :full_rq_pfx OR UPPER(m.class_category) LIKE :full_rq_pfx THEN 2
                ELSE 3
            END ASC,
            m.cos_sec ASC NULLS LAST,
            m.prf_group ASC NULLS LAST,
            m.item_code ASC NULLS LAST,
            m.census_no ASC NULLS LAST,
            m.nomen ASC NULLS LAST"""
    elif text_val:
        ...
        order_sql = """ORDER BY
            CASE
                WHEN UPPER(m.material_no) = :full_txt_exact OR UPPER(m.census_no) = :full_txt_exact OR UPPER(m.nomen) = :full_txt_exact OR UPPER(m.cat_part_no) = :full_txt_exact OR UPPER(m.class_category) = :full_txt_exact THEN 1
                WHEN UPPER(m.material_no) LIKE :full_txt_pfx OR UPPER(m.census_no) LIKE :full_txt_pfx OR UPPER(m.nomen) LIKE :full_txt_pfx OR UPPER(m.cat_part_no) LIKE :full_txt_pfx OR UPPER(m.class_category) LIKE :full_txt_pfx THEN 2
                ELSE 3
            END ASC,
            m.cos_sec ASC NULLS LAST,
            m.prf_group ASC NULLS LAST,
        ...
        order_sql = "ORDER BY m.cos_sec ASC NULLS LAST, m.prf_group ASC NULLS LAST, m.item_code ASC NULLS LAST, m.census_no ASC NULLS LAST, m.nomen ASC NULLS LAST"
...
def search_mlccs(
    ...
    from_sql = """
        FROM MMS_MLCCS_EQPT_MASTER m
        LEFT JOIN (
            SELECT item_code, MAX(item_name) AS item_name
            FROM MMS_PRF_GRP_MSTR
            WHERE item_code IS NOT NULL
            GROUP BY item_code
        ) p ON UPPER(TRIM(m.item_code)) = UPPER(TRIM(p.item_code))
    """

    count_sql = f"SELECT COUNT(m.id) AS cnt {from_sql} {where_sql}"
