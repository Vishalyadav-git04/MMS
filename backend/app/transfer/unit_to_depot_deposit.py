...
Queries and updates MMS_UNIT_MSTR_DETL table.
Maps parent units with MMS_ORBAT_UNIT_DETL and domain codes with MMS_DOMAIN_VALUES.
Maps receiving units with MMS_ORBAT_UNIT_DETL.
Maps PRF code with MMS_PRF_GRP_MSTR table to get PRF group.
"""
...
def _get_tfr_status_code(session: Session) -> str:
    """Retrieve TFRSTATUS code value for deposited/transferred equipment."""
    code = session.scalar(
        select(DomainValue.code_value).where(
            func.replace(func.upper(DomainValue.domain_name), "_", "") == "TFRSTATUS",
            or_(
                func.upper(DomainValue.label_name).like("%DEPOSIT%"),
                func.upper(DomainValue.label_name).like("%TRANSFER%"),
                func.upper(DomainValue.code_value).like("%DEP%"),
    ...
    return (code or "DEPOSITED").strip()
...
class DepositSubmitOut(BaseModel):
    count: int
...
def get_parent_units(session: Session = Depends(get_db_session)) -> list[ParentUnitOut]:
    """Get distinct parent units (to_sus_no) from MMS_UNIT_MSTR_DETL mapped with MMS_ORBAT_UNIT_DETL."""
    approved_codes = _get_approved_op_codes(session)
    suses = session.scalars(
    ...
    clean_suses = [s.strip().upper() for s in suses if s and s.strip()]
    if not clean_suses:
        return []
...
def get_parent_holding_types(
    ...
    """Get distinct type_of_hldg from MMS_UNIT_MSTR_DETL for selected parent unit."""
    approved_codes = _get_approved_op_codes(session)
...
def get_parent_eqpt_types(
    ...
    """Get distinct type_of_eqpt from MMS_UNIT_MSTR_DETL for parent unit and holding type."""
    approved_codes = _get_approved_op_codes(session)
    sus = parent_sus_no.strip().upper()
    stmt = select(func.distinct(UnitMasterDetail.type_of_eqpt)).where(
        func.upper(UnitMasterDetail.to_sus_no) == sus,
        UnitMasterDetail.type_of_eqpt.is_not(None),
        func.upper(func.trim(func.coalesce(UnitMasterDetail.op_status, ""))).in_(approved_codes),
    )
    if holding_type and holding_type.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.type_of_hldg) == holding_type.strip().upper())
...
def get_parent_prf_groups(
    ...
    """Get distinct prf_code from MMS_UNIT_MSTR_DETL mapped with MMS_PRF_GRP_MSTR."""
    approved_codes = _get_approved_op_codes(session)
    sus = parent_sus_no.strip().upper()
    stmt = select(func.distinct(UnitMasterDetail.prf_code)).where(
        func.upper(UnitMasterDetail.to_sus_no) == sus,
    ...
    if eqpt_type and eqpt_type.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.type_of_eqpt) == eqpt_type.strip().upper())
    ...
    # Map PRF_CODE from MMS_UNIT_MSTR_DETL with MMS_PRF_GRP_MSTR to get PRF_GRP
    int_codes = []
    ...
    # Fallback to MLCCS Equipment Master
...
def get_parent_nomenclatures(
    ...
    """Get distinct census_no from MMS_UNIT_MSTR_DETL mapped with MMS_MLCCS_EQUIPMENT_MASTER."""
    approved_codes = _get_approved_op_codes(session)
    sus = parent_sus_no.strip().upper()
    stmt = select(func.distinct(UnitMasterDetail.census_no)).where(
        func.upper(UnitMasterDetail.to_sus_no) == sus,
    ...
    if prf_code and prf_code.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.prf_code) == prf_code.strip().upper())
...
def get_receiving_units(
    ...
    """Get active receiving depots/units from MMS_ORBAT_UNIT_DETL."""
...
def get_receiving_holding_types(
    ...
    """Get receiving holding types from MMS_DOMAIN_VALUES where code_value is like %G% for domain_name=TYPE_OF_HOLDING."""
    rows = session.scalars(
        select(DomainValue)
        .where(
            func.replace(func.upper(DomainValue.domain_name), "_", "") == "TYPEOFHOLDING",
            func.upper(DomainValue.code_value).like("%G%"),
        )
    ...
    if not rows:
        ...
        g_rows = [r for r in rows if r.code_value and "G" in r.code_value.upper()]
        if g_rows:
            rows = g_rows
    ...
    if not res:
        res = [OptionOut(value="G", label="General Holding")]
...
def get_regn_list(
    ...
    """Get registration numbers from MMS_UNIT_MSTR_DETL for specified parent and filters."""
    approved_codes = _get_approved_op_codes(session)
    sus = parent_sus_no.strip().upper()
    stmt = select(UnitMasterDetail.eqpt_regn_no).where(
        func.upper(UnitMasterDetail.to_sus_no) == sus,
    ...
    if census_no and census_no.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.census_no) == census_no.strip().upper())
...
@router.post("/transfer", response_model=DepositSubmitOut)
def submit_deposit(
    body: DepositSubmitIn,
    session: Session = Depends(get_db_session),
) -> DepositSubmitOut:
    """Submit Unit to Depot Deposit transaction, transforming records in MMS_UNIT_MSTR_DETL."""
    parent_sus = body.parent_sus_no.strip().upper()
    receiving_sus = body.receiving_sus_no.strip().upper()

    if not body.regn_numbers:
        raise HTTPException(status_code=400, detail="No registration numbers provided for deposit")
    ...
    if body.type_of_hldg and body.type_of_hldg.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.type_of_hldg) == body.type_of_hldg.strip().upper())
    if body.type_of_eqpt and body.type_of_eqpt.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.type_of_eqpt) == body.type_of_eqpt.strip().upper())
    if body.prf_code and body.prf_code.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.prf_code) == body.prf_code.strip().upper())
    if body.census_no and body.census_no.strip():
        stmt = stmt.where(func.upper(UnitMasterDetail.census_no) == body.census_no.strip().upper())

    rows = session.scalars(stmt).all()
    if not rows:
        raise HTTPException(status_code=404, detail="No approved equipment records found for deposit")
    ...
    for r in rows:
        ...
        r.iv_no = body.rv_no.strip()
        r.iv_date = rv_dt
        if body.upload_rv:
            r.upload_iv = body.upload_rv.strip()
        if r.eqpt_regn_no:
            transferred.append(r.eqpt_regn_no)

    session.flush()
    return DepositSubmitOut(count=len(transferred), transferred_regns=transferred)
