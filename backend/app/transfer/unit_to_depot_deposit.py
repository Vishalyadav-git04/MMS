"""Unit to Depot Deposit — Weapon → EQPT Transfer using Native SQL."""
...
def _get_tfr_status_code(session: Session) -> str:
    sql = """
        ...
        AND (UPPER(label_name) LIKE '%DEPOSIT%' OR UPPER(label_name) LIKE '%TRANSFER%' OR UPPER(code_value) LIKE '%DEP%' OR UPPER(code_value) LIKE '%TRANS%' OR UPPER(code_value) LIKE '%TFR%')
    """
    ...
    return str(val or "DEPOSITED").strip()
...
def get_receiving_holding_types(
    ...
    rows = fetch_all(
        ...
        AND (UPPER(code_value) LIKE '%G%' OR UPPER(label_name) LIKE '%GEN%')
        ORDER BY LPAD(COALESCE(disp_order, '9999'), 10, '0'), label_name
        """,
    )
    if not rows:
        return [OptionOut(value="G", label="General Holding")]
